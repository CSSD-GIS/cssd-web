<template>
  <div class="rightFather">
    ...
    <div class="border">
      ...
      <!-- <span class="classroom">S101半球 </span>
     
         <span class="classroom">S102半球 </span>
       <span class="li">离线</span>
          <span class="classroom">S103半球 </span>
       <span class="zai">在线</span>
            <span class="classroom">S104半球 </span>
       <span class="li">离线</span>
        <span class="classroom">S105半球 </span>
       <span class="zai">在线</span>
        <span class="classroom">S106半球 </span>
       <span class="zai">在线</span>
        <span class="classroom">N111半球 </span>
       <span class="li">离线</span>
        <span class="classroom">N112半球 </span>
       <span class="zai">在线</span>
        <span class="classroom">N113半球 </span>
       <span class="zai">在线</span>
        <span class="classroom">N114半球 </span>
       <span class="li">离线</span>
        <span class="classroom">N115半球 </span>
       <span class="zai">在线</span>
       <span class="classroom">N116半球 </span>
       <span class="li">离线</span> -->
     <span class="classroom"  v-for="(val, key) in items1" :key="key" @click="choice(key)"
        >{{ `${key} ` + "半球" }}
        <span v-if="show(key)" id="stateoff" >{{statusoff}}</span>
        <span v-else id="stateon" >{{status}}</span>
      </span>

      <!-- <div v-for="u in itemarr" v-bind:key="u">{{u}}</div> -->
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    // let items1 = config.north.front.floor1;
    // let newStr1 = JSON.parse(JSON.stringify(items1));
    // let arr1 = [];
    // // 循环对象，把每一项push到数组中
    // for (let i in newStr1) {
    //   arr1.push(newStr1[i]);
    // }

    // let p1 = parseInt(this.$route.query.id1);

    // //------------------------------二层------------------------------
    // this.id2 = this.$route.query.id2;
    // let items2 = config.north.front.floor2;
    // let newStr2 = JSON.parse(JSON.stringify(items2));
    // let arr2 = [];
    // // 循环对象，把每一项push到数组中
    ...
    // let p2 = parseInt(this.$route.query.id2);
  },
  methods: {
    handleChange(v) {
